        ...
        {loading ? (
          ...
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(170px,1fr))', gap: 14, marginBottom: 22 }}>
              <BigStat icon={<LinkIcon size={20} color="#15141c" strokeWidth={2.5} />} label="Total Links"      value={data.totalLinks}            color={V}    delay={0}   sub="in workspace" />
              <BigStat icon={<Pointer size={20} color="#15141c" strokeWidth={2.5} />} label="Total Clicks"     value={total.toLocaleString()}     color={GRN}  delay={60}  sub="all time" />
              <BigStat icon={<TrendingUp size={20} color="#15141c" strokeWidth={2.5} />} label="Avg Clicks/Link"  value={data.avgClicksPerLink}      color={AMB}  delay={120} sub="per link" />
              <BigStat icon={<CheckCircle size={20} color="#15141c" strokeWidth={2.5} />} label="Active Links"     value={data.activeLinks}           color={BLU}  delay={180} sub={`of ${data.totalLinks}`} />
              <BigStat icon={<Clock size={20} color="#15141c" strokeWidth={2.5} />} label="Expired Links"    value={data.expiredLinks}          color="#9f67f5" delay={240} sub="past deadline" />
            </div>

            {/* ── MAIN CHART + TOP LINKS ── */}
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 20, marginBottom: 20 }}>
              {/* Clicks + Links created over time */}
              <ChartCard title="Clicks & links created" sub="Daily trend over selected period" icon={<BarChart2 size={20} color="#15141c" strokeWidth={2.5} />} delay={80}
                right={
              ...
              {/* Top 5 links */}
              <ChartCard title="Top links" sub="Highest click count" icon={<Trophy size={20} color="#15141c" strokeWidth={2.5} />} delay={100}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 20, marginBottom: 20 }}>
              ...
              {/* Countries */}
              <ChartCard title="Top countries" sub="All-time geo breakdown" icon={<Globe size={20} color="#15141c" strokeWidth={2.5} />} delay={160}>
              ...
              {/* Device donut */}
              <ChartCard title="Device types" sub="Desktop vs mobile vs tablet" icon={<Smartphone size={20} color="#15141c" strokeWidth={2.5} />} delay={200}>
                  ...
                  <>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                      {data.byDevice.map(({device,count},i) => {
                        const pct = total ? Math.round((count/total)*100) : 0
                        const icons = { desktop:<Monitor size={14} color="#15141c" strokeWidth={2.5} />, mobile:<Smartphone size={14} color="#15141c" strokeWidth={2.5} />, tablet:<Tablet size={14} color="#15141c" strokeWidth={2.5} /> }
                        return (
                          <div key={device||i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                              ...
                              <span style={{ fontFamily: "'Space Grotesk',sans-serif", fontSize: '0.825rem', color: INK }}>{icons[device?.toLowerCase()]||<Monitor size={14} color="#15141c" strokeWidth={2.5} />} {device?device[0].toUpperCase()+device.slice(1):'Unknown'}</span>
                            </div>
              ...
              {/* Browser + Referrers */}
              <ChartCard title="Traffic sources" sub="Browsers & referrers" icon={<LinkIcon size={20} color="#15141c" strokeWidth={2.5} />} delay={240}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: 20 }}>
              ...
              {/* All links table */}
              <ChartCard title="All links performance" sub="Click breakdown across your workspace" icon={<ClipboardList size={20} color="#15141c" strokeWidth={2.5} />} delay={280}>
                {urls.length > 0 ? (
                          ...
                          return (
                            <tr key={u.id} style={{ borderBottom: `1px solid ${LINE}`, cursor: 'pointer', transition: 'background .15s' }}
                              ...
                              <td style={{ padding: '10px 13px', whiteSpace: 'nowrap' }}>
                                <span style={{ fontFamily: "'Fragment Mono',monospace", fontSize: '0.8rem', fontWeight: 600, color: V }}>tinyhop-url/{u.shortCode}</span>
                              </td>
              ...
              <ChartCard title="Recent activity" sub="Latest clicks across all links" icon={<Zap size={20} color="#15141c" strokeWidth={2.5} />} delay={300}>
                {data.recentActivity?.length > 0 ? (
                  ...
                  <div style={{ textAlign: 'center', padding: '32px 0' }}>
                    <div style={{display:"flex", justifyContent:"center", marginBottom:8}}><Inbox size={28} color="#15141c" strokeWidth={2.5} /></div>
